...
| 面談相手 / アプローチ | アクション名 / インプット共有カード | 解放条件 | 得られる効果・フラグ |
| :--- | :--- | :--- | :--- |
| **現場PL** | **📝 【会議の論点・アジェンダ整理】直近予定の会議に向けた論点・質問整理** | 初期（自席・開発チームフロア） | 直近予定会議（または次回会議）に適用、`CARD_AGENDA` (事前アジェンダカード獲得) |
| **顧客** | **🏃‍♂️ 【アポ先行（アジェンダなし）】急ぎで会議調整・開催** | 初期（アジェンダ未作成） | 🔴 キーマン不在ペナルティ（顧客満足度 -5%、要件明確化 +0、時間消費） |
| **顧客** | **🗣️ 【アジェンダ提示会議】整理した議題を提示しキーマン同席打ち合わせ** | `CARD_AGENDA` 獲得済み | 🟢 キーマン同席成功（顧客満足度 +5%、要件明確化 +2） |
...
1. **Step 1-1: 上司からの業務アサイン（状況把握）**
   - 上司から案件概要、納期、顧客タイプ、現場（PL）のコンディションを把握。
...
## 6. 🌐 Web ADV版におけるキックオフ下準備 ＆ PMO軍師ガイド統合

Web ADV版では、以下の構成でキックオフ下準備から開発スプリントへ移行します：

1. **下準備フェーズ (`KICKOFF_PREP`)**:
   - 各フロア（顧客・上司・現場）で下準備アクション（技術リスク精査、過去教訓共有、QCD合意など）を実行。
   - **`🦉 PMOのアドバイス`** が常置され、やるべき下準備や順序コンボの示唆、および Day 3 以降のタイムリミット警鐘（「事前準備超過による納期圧迫」）が出力される。
2. **キックオフ決起 (`team_kickoff_rally`)**:
   - 開発チームフロアで `🚀 チームキックオフ決起（方針宣言）` を実行。
   - 下準備で手に入れた根拠数（獲得フラグ）に応じたキックオフ評価ランク（**S/A/B/C**）を決定。
   - 判定結果に応じて初期のチーム士気・安全度ボーナスが付与され、**開発スプリントフェーズ (`DEVELOPMENT_SPRINT`)** へ突入する。
