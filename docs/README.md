...
### ☕ [docs/spec/ (カフェ開店編・最新現行仕様)](spec/overview.md)
専門用語を使わず、身近な「カフェの新規開店」を題材にPMの本質（段取り・合意形成・人間関係）を学ぶ最新仕様書群です。
* **[spec/overview.md (全体概要)](spec/overview.md)**: カフェオープン版のゲームシステム、PM概念対応表、登場人物モデル、コアループ
...
### 🛠 [AI開発ガイドライン & 開発ハーネス規約 (AGENTS.md)](../AGENTS.md)
プロジェクト共通の開発規約・Git運用・Issue駆動ワークフロー・AI開発ガードレールです。
* [AGENTS.md (リポジトリ直下)](../AGENTS.md): Git / GitHub / Issue 運用規約 ＆ ガイドライン

---

### 📦 [docs/old/ (過去バージョンアーカイブ)](old/README.md)
過去に検討・試行錯誤された旧バージョンの仕様・設計アーカイブです。
* **[old/v2/](old/v2/README.md)**: IT開発編ゼロベース仕様アーカイブ
* **[old/v1/](old/v1/README.md)**: 初期バージョン（キックオフ3AP制やWF/アジャイル対比等）のアーカイブ
