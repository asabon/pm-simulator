...
## ⚖️ PMとPLの責務分離（Why / What vs How）

本作では、現実のプロジェクトマネジメントの本質を体験するため、**「店長（PM）」と「チーフ（PL）」の役割分担**を明確に定義しています。

```mermaid
flowchart TD
    subgraph PM_Scope ["🎯 店長（PM）の主責務"]
        Why["💡 <b>Why（目的・価値）</b><br/>・なぜこのカフェを作るのか？<br/>・経営目標・予算・期日<br/>・誰を喜ばせるのか？"]
        What["📋 <b>What（スコープ・仕様）</b><br/>・何を提供するのか？（メニュー絞り込み）<br/>・何を諦めるのか？（トレードオフ判断）<br/>・合格基準（レセプション受け入れ条件）"]
    end

    subgraph PL_Scope ["👨‍🍳 チーフ料理長（PL）の主責務"]
        How["🛠 <b>How（実現手法・設計）</b><br/>・どう実現するのか？（動線・厨房レイアウト）<br/>・どう調理するのか？（レシピ・マニュアル策定）<br/>・技術的フィジビリティ・工数見積もり"]
        Who["👥 <b>Who / Execution（体制・作業）</b><br/>・誰がどの仕込みを担当するか<br/>・現場トレーニングと調理オペレーション"]
    end

    Why --> What
    What <-->|スコープ調整 / フィジビリティ検証| How
    How --> Who
```

* **店長（PM）の役割（Why & What）**:
  * 「なぜこのカフェを開くのか（Why）」をオーナーと握り、「何を提供し、何を削るか（What）」を最終決定する。
  * チーフの技術的知見（How）を尊重し、無理な納期や仕様を押し付けず、現場が自走できる土台を整える。
* **チーフ料理長（PL）の役割（How & Execution）**:
  * 店長が決めた What に対し、「厨房の物理的制約やスタッフのスキルでどう実現するか（How）」を具体化・見積もる。
  * 現場目線でのリスク（「この厨房ではパスタとピザの両立は無理です」など）をPMに進言する。

---

## 🔍 PMが「What（何を作るか）」を決めるのが難しい4大ケース

PMの仕事は「机の上で独断でWhatを決めること」ではありません。現実のプロジェクト同様、**Whatを決めるのが困難な4つの典型パターン**が存在し、プレイヤーは適切な対話と探索を通じてWhatを削り出していく必要があります。

```mermaid
flowchart TB
    Issue1["① 利害・制約の対立<br/>(政治・ビジネス調整)"] -->|オーナー進捗報告 & 保健所事前相談| Solved1["合意されたWhat<br/>(法規適合メニュー)"]
    Issue2["② 物理的・技術的制約<br/>(Howからの逆流)"] -->|チーフと状況相談| Solved2["実現可能なWhat<br/>(絞り込まれた品数)"]
    Issue3["③ 専門知識の不足<br/>(エキスパート依存)"] -->|調理レシピ策定を依頼| Solved3["具体化されたWhat<br/>(標準マニュアル)"]
    Issue4["④ 不確実性・未検証<br/>(やってみないと不明)"] -->|オペレーション練習 & レセプション| Solved4["検証済みのWhat<br/>(本番運用仕様)"]
```

| 困難パターン | カフェオープン編での具体例 | 求められるPMの行動（コマンド） |
| :--- | :--- | :--- |
| **① 利害・制約の対立<br>（政治・法規制調整）** | **オーナーの思いつき vs 保健所の衛生基準**<br>オーナーが生肉料理やペット同伴を要求するが、保健所基準で営業停止リスクがある。 | **【オーナー進捗報告】＆【保健所への事前相談】**<br>法規の壁を根拠にオーナーと交渉し、合意可能な代替メニュー（What）を再定義する。 |
| **② 技術的・物理的制約<br>（現場Howからの逆流）** | **狭い厨房・設備限界 vs 多彩なメニュー**<br>店長が「パスタもケーキもピザも全品出したい」と考えても、コンロ数や厨房面積（How）から物理的にパンクする。 | **【チーフと状況相談】＆【メニュー絞り込み】**<br>チーフからのフィジビリティ進言を受け入れ、看板メニューに品数を絞り込む（Whatのトレードオフ）。 |
| **③ 専門知識の不足<br>（現場エキスパート依存）** | **看板商品の味・分量・仕込み手順**<br>店長が「美味しいオムライスを出す」と決めても、具体的な調味料比率や加熱時間はチーフの知見がないと確定できない。 | **【調理レシピ策定】（チーフへの権限委譲）**<br>店長が勝手に決めつけず、チーフをリスペクトしてマニュアル化を依頼・支援する。 |
| **④ 不確実性・未検証<br>（探索・アジャイル検証）** | **注文ラッシュ時のリアルな動線詰まり**<br>図面や紙のレシピ上は完璧に見えても、実際にスタッフが動けるかはやってみないとわからない。 | **【オペレーション練習】＆【関係者レセプション】**<br>模擬営業（テスト）を実施してボトルネックを洗い出し、盛り付けや手順（What）を微調整する。 |
...
| ステップ | 担当領域 | 正しいPMの行動（ベストプラクティス） | 段取りを飛ばした時の失敗結果（アンチパターン） |
| :--- | :--- | :--- | :--- |
| **① コンセプト確認 ＆<br/>オーナー合意** | **Why** | ターゲット客層や予算を明確にし、**オーナーから書面で合意を取り付ける** | 現場だけで進め、完成直前にオーナーから「こんな地味なカフェにする気じゃなかった」と全取っ替え |
| **② メニュー確定 ＆<br/>許可基準確認** | **What** | 保健所基準やチーフの意見を踏まえて看板メニューを絞り込み、提供範囲を確定 | メニューを増やしすぎて仕込み不能に。保健所ルール無視で手洗い場の追加工事が発生 |
| **③ 動線 ＆ レシピ設計** | **How** | チーフに厨房のすれ違い動線と誰でも作れるレシピマニュアルを策定・設計してもらう | レシピなしで各自が勘で調理。動線が悪くて厨房内でスタッフが激突 |
| **④ 体制・シフト構築** | **Who** | 必要人数を算出して採用し、役割分担とトレーニング計画を立てる | 人手不足のまま突入して既存スタッフが過労で退職。役割分担が曖昧で注文が放置される |
| **⑤ 工事・練習推進** | **Build** | チーフと連携して内装工事を進め、スタッフに模擬注文の練習をさせる | 練習なしで本番を迎え、伝票が紛失・料理提供に40分かかって客が激怒 |
| **⑥ 試食会・本オープン** | **Deliver** | 関係者を招いたレセプションで課題を洗い出し、万全の状態でグランドオープン | チェックなしでオープンし、レジが動かない・お皿が足りない大パニックで即日炎上 |
