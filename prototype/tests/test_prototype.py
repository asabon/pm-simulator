...
def test_developer_resolution_display():
    dev = Developer("dev1", "テスト太郎", 1.0, 0.02, 10000, [], age=25, resolution=0)
...
def test_urgent_events():
    ...
    project = Project("テストPJ", 1000000, 4, customer)
    pm = PM()
    dev = Developer("dev1", "テスト開発者", 1.0, 0.02, 10000, [], age=30)
...
def test_process_yearly_closing():
    pm = PM()
    dev_young = Developer("dev_y", "若手", 1.0, 0.02, 10000, [], age=24, resolution=0)
    dev_old = Developer("dev_o", "ベテラン", 1.5, 0.01, 20000, [], age=64, resolution=1)
